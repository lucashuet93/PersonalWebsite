...
`;
		...
		return (
			<div className="ms-font-m ms-fontWeight-semilight blogPost">
				...
				<p>Any post requests made to the Logic App endpoint with a body that matches that schema definition will now successfully trigger emails! Incredibly easy, right? If Logic Apps isn’t aware of the structure of the JSON object returned from a step in the workflow, however, it can be a bit trickier. </p>
				...
				<p>Logic Apps can intelligently search your Azure resources and find available queues, so this Logic App will check for items in my <span className="ms-fontWeight-semibold">email</span> queue every <span className="ms-fontWeight-semibold">30 seconds</span>.</p>
